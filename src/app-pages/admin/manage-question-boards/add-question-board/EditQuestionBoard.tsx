...
import { useGetQuestionBoardById } from "@/hooks/queries/useGetAllQuestionBoards";
import { useUpdateQuestionBoard } from "@/hooks/mutations/useUpdateQuestionBoard";
...
const EditQuestionBoards = () => {
  const searchParams = useSearchParams();
  const questionBoardId = searchParams.get("questionBoardId") || "";
  const { data: fetchedQB, isLoading } = useGetQuestionBoardById(questionBoardId);
  ...
  const subTopicOptions: { name: string; value: string }[] = Array.isArray(
    ...
    const { mutate,isPending  } = useUpdateQuestionBoard()

  const mapApiToFormValues = React.useCallback((api: any): FormValues => {
    const meta = api?.meta || {};
    const groups: any[] = Array.isArray(api?.questionBoardData) ? api.questionBoardData : [];

    // sort by number just in case
    groups.sort((a, b) => (a?.number || 0) - (b?.number || 0));

    const formQuestions = groups.map((g) => {
      const questionsArr: any[] = Array.isArray(g?.questions) ? g.questions : [];
      if (g?.isQuestionGroup) {
        return {
          layoutType: "PARAGRAPH",
          mainQuestionTitle: g?.questionTitle ?? '',
          mainQuestionDescription: g?.paragraph || "",
          paragraph: g?.paragraph || "",
          subQuestions: questionsArr
            .sort((a, b) => (a?.questionNumber || 0) - (b?.questionNumber || 0))
            .map((q) => {
              const isTF = q?.questionType === 'TRUE_FALSE';
              const apiOptions = Array.isArray(q?.options) ? q.options : [];
              const answerOption = apiOptions.find((o: any) => o?.isCorrect);
              const normalizedOptions = isTF
                ? [
                    { id: crypto.randomUUID(), option: 'True', isCorrect: answerOption?.option === 'True' },
                    { id: crypto.randomUUID(), option: 'False', isCorrect: answerOption?.option === 'False' },
                  ]
                : apiOptions.map((o: any) => ({ id: crypto.randomUUID(), option: o?.option || '', isCorrect: !!o?.isCorrect }));
              return {
                question: q?.question || "",
                ...
                answer: isTF ? (answerOption?.option === 'True' || answerOption?.option === 'False' ? answerOption?.option : undefined) : (answerOption?.option || undefined),
                image: q?.questionImage || undefined,
              };
            }),
        } as ParagraphQuestion;
      }
      // WITHOUT_PARAGRAPH group contains a single question in questions[0]
      const single = questionsArr[0] || {};
      const isTF = single?.questionType === 'TRUE_FALSE';
      const apiOptions = Array.isArray(single?.options) ? single.options : [];
      const answerOption = apiOptions.find((o: any) => o?.isCorrect);
      const normalizedOptions = isTF
        ? [
            { id: crypto.randomUUID(), option: 'True', isCorrect: answerOption?.option === 'True' },
      ...
      return {
        ...
        questionType: (single?.questionType as any) || "MCQ",
        questionDescription: single?.questionDescription || "",
        explanation: single?.explanation || "",
        options: normalizedOptions,
        answer: isTF ? (answerOption?.option === 'True' || answerOption?.option === 'False' ? answerOption?.option : undefined) : (answerOption?.option || undefined),
        image: single?.questionImage || undefined,
      } as WithoutParagraphQuestion;
    });

    return {
      questionBoardTitle: meta?.questionBoardTitle || "",
      questionDescription: meta?.questionDescription || "",
      boardImage: meta?.questionBoardImgae || undefined,
      topic: meta?.topic?._id || meta?.topicId || "",
      subTopic: meta?.subTopic?._id || meta?.subTopicId || "",
      grade: meta?.grade?._id || meta?.gradeId || "",
      subject: meta?.subject || "",
      durationTime: String(meta?.durationTime || "30"),
      level: (meta?.level as any) || "",
      passPacentage: typeof meta?.passPacentage === "number" ? meta.passPacentage : 40,
      country: meta?.country || "India",
      // boardImage: meta?.questionBoardImgae || undefined,
      status: (meta?.status as any) || "DRAFT",
      questions: formQuestions.length ? formQuestions : [emptyWithoutParagraph()],
    };
  }, []);

  const computedInitialValues = React.useMemo<FormValues>(() => {
    if (!fetchedQB) return initialValues;
    try {
      const vals = mapApiToFormValues(fetchedQB);
      return vals;
    } catch (_) {
      return initialValues;
    }
  }, [fetchedQB, mapApiToFormValues]);

  React.useEffect(() => {
    if (computedInitialValues?.topic) setSelectedTopicId(computedInitialValues.topic);
    if (computedInitialValues?.grade) setSelectedGradeId(computedInitialValues.grade);
    if (computedInitialValues?.subject) setSelectedSubjectName(computedInitialValues.subject);
  ...
  return (
    <div>
      <Formik
        ...
        onSubmit={(vals) => {
          const data = toQuestionBoardApiPayload(vals);
          const payload = {
            ...data,
...
export default EditQuestionBoards;
