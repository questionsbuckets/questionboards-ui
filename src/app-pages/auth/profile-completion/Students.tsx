...
import { useAddStudentAccount } from "@/hooks/mutations/useAddParentsAccount";
function StudentsProfile() {
  const { mutateAsync: addStudentsAccount, isPending } = useAddStudentAccount();
  ...
  const formik = useFormik<ParentProfileFormValues>({
    ...
    onSubmit: async (values) => {
      ...
      try {
        const res = await addStudentsAccount(formdata);
        toast.success(res.message || "Parents profile Added successfully!");
        router.push("/home");
      } catch (error: any) {
        console.log("🚀 ~ StudentsProfile ~ error:", error);

        toast.error(
          error?.response?.data?.message || "Students profile Added failed!"
        );
      }
    },
  });
